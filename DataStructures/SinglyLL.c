...
void main(){
    ...
    int choice, n, ch, val;
    do{
        printf("Enter Choice:\n1. Insert    2. Delete    3. Sort    4. Search    5. Reverse    6. Display   7. Exit\n");
        scanf("%d", &choice);
        switch (choice)
        {
        case 1:
...
node* createLL(node* head){
    ...
    do{
        ...
        newNode->data=num;
        if(head==NULL){newNode->next=NULL; head=newNode;}
        else{
            temp=head;
...
node* delAtEnd(node* head){
    if(head==NULL){printf("UNDERFLOW!\n");}
        ...
        node *temp1, *temp2;
        temp1=temp2=head;
        if(head->next==NULL){head=NULL; free(temp1); return head;}
        while(temp2->next!=NULL){    /*temp1->next->next!=NULL o/s while: temp2=temp1->next*/
            temp1=temp1->next;
            temp2=temp1->next;
        }
        temp1->next=NULL;
